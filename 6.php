...
<?php
$concert = mysqli_query($con, "SELECT title FROM concerts WHERE id = '6'");
$concert = mysqli_fetch_assoc($concert);

$condesc = mysqli_query($con, "SELECT description FROM concerts WHERE id = '6'");
$condesc = mysqli_fetch_assoc($condesc);

$condate = mysqli_query($con, "SELECT concert_date FROM concerts WHERE id = '6'");
$condate = mysqli_fetch_assoc($condate);

$conprice = mysqli_query($con, "SELECT baseprice FROM concerts WHERE id = '6'");
...
<div class="q1"> <?php echo($condate["concert_date"]); ?> </div>
 ...
 <a type="submit" class="button" href="booking.php?concert_id=6">Замовити квиток</a></div>
