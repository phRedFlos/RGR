...
<?php
$concert = mysqli_query($con, "SELECT title FROM concerts WHERE id = '1'");
$concert = mysqli_fetch_assoc($concert);

$condesc = mysqli_query($con, "SELECT description FROM concerts WHERE id = '1'");
$condesc = mysqli_fetch_assoc($condesc);

$condate = mysqli_query($con, "SELECT concert_date FROM concerts WHERE id = '1'");
$condate = mysqli_fetch_assoc($condate);

$conprice = mysqli_query($con, "SELECT baseprice FROM concerts WHERE id = '1'");
$conprice = mysqli_fetch_assoc($conprice);
?>

<h1><?php echo($concert["title"]); ?> </h1>
  <p ><img  width = 600 px height= auto src="11.jpg" class="rightimg">
...
<div class="q1"> <?php echo($condate["concert_date"]); ?> </div>
 ...
 <a type="submit" class="button" href="booking.php?concert_id=1">Замовити квиток</a></div>
